...
import com.foxconn.emm.dao.SQLiteAssetHelper.SQLiteAssetException;


/**
 * Compare paths by their upgrade version numbers, instead of using alphanumeric
 * comparison on plain file names. This prevents the upgrade scripts from being
 * applied out of order when they first move to double-, triple-, etc. digits.
 * <p>
 * For example, this fixes an upgrade that would apply 2 different upgrade files
 * from version 9 to 11 (<code>..._updated_9_10</code> and
...
class VersionComparator implements Comparator<String> {
	 ...
	 * database names used are the same, as this function only compares the two
	 * version numbers.
	 * 
	 * @param file0
	 ...
	 * @return an integer < 0 if file0 should be applied before file1, 0 if they
	 *         are equal (though that shouldn't happen), and > 0 if file0 should
	 *         be applied after file1.
	 * 
