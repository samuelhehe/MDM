...
public class FrgWhiteApp extends Fragment implements IMessageReceiveCallback {

	private static final int LOAD_COMPLETE = 1;

	private static final int LOAD_FAIL = 0;

	private ListView frg_limit_app_white_lv;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		rootView = inflater.inflate(R.layout.frg_white_app, container, true);
		initView();
		initData();
		return rootView;
	}
	...
	public void onAttach(Activity activity) {
		...
		L.d(this.getClass(), "onAttach FrgWhiteApp ");

	}

	private void initView() {
		if (rootView != null) {
			frg_limit_app_white_lv = (ListView) rootView
					.findViewById(R.id.frg_limit_app_white_lv);
			frg_limit_app_white_lv.setVisibility(View.INVISIBLE);
		}
	...
	protected Handler handler = new Handler() {
		...
		public void handleMessage(android.os.Message msg) {
			...
			switch (msg.what) {
			case LOAD_COMPLETE:
				if (frg_limit_app_white_lv != null) {
					datas = (List<InstalledAppInfo>) msg.obj;
					adapter = new LimitListAdapter(getActivity(), datas,
							R.layout.frg_limitlist_item);
					frg_limit_app_white_lv.setAdapter(adapter);
					frg_limit_app_white_lv.setVisibility(View.VISIBLE);
				}
				break;

			case LOAD_FAIL:

//				ToastUtils.show(getActivity(), "白名单列表为空");
				break;
			}
		};
	};
			...
			public void run() {
				try {
					...
					List<InstalledAppInfo> datas = blackListDao.getFilteredPackageLimitList(blackListDao
							.getLimitListbyType(EMMContants.LIMITLIST_CONTANT.LT_WHITE_LIST));
