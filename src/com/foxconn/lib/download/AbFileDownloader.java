...
public class AbFileDownloader {
	...
	public AbFileDownloader(Context context, DownFile downFile, int threadNum) {
		try {
			...
			// 构建保存文件
			// String fileName =
			// AbFileUtil.getRealFileNameFromUrl(mDownFile.getDownUrl());
			String fileName = null;
			if (TextUtils.isEmpty(fileName)) {
				fileName = downFile.getName() + downFile.getSuffix();
			}

			if (downFile.getDownPath().equals("")
					|| downFile.getDownPath().equals(null)) {
				saveFile = new File(
						EMMContants.LocalConf.getEMMLocalHost_dirPath()
								+ EMMContants.LocalConf.download_dirpath
								+ fileName);
			} else {
				saveFile = new File(downFile.getDownPath());
			}
