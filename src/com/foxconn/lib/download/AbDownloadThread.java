...
public class AbDownloadThread extends Thread {
	 ...
	 *            the save file
	 */
	public AbDownloadThread(AbFileDownloader downloader, DownFile downFile,
			File saveFile) {
	...
	public void run() {
		...
		if (mDownFile.getDownLength() < mDownFile.getTotalLength()) {
			try {
				...
				HttpURLConnection http = (HttpURLConnection) mUrl
						.openConnection();
				...
				while (flag
					...
					if (offset != 0) {
						...
						if (D)
							Log.d(TAG,
									"DownLength:" + mDownFile.getDownLength()
											+ "/" + mDownFile.getTotalLength());
						downloader.update(mDownFile);

						if (mDownFile.getDownLength() == mDownFile
								.getTotalLength()) {
							this.finish = true;
							flag = false;
